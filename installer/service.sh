...
MODDIR="${0%/*}"
case "$MODDIR" in
    */installer) MODDIR="${MODDIR%%/installer}" ;;
    */scripts) MODDIR="${MODDIR%%/scripts}" ;;
...
# =============================================================================
# Setup wget wrapper for busybox (channel analysis requires HTTP client)
# =============================================================================
setup_wget_wrapper() {
    WGET_WRAPPER="/data/local/tmp/wget"
    
    # Check if busybox is available
    if ! command -v busybox >/dev/null 2>&1; then
        echo "[SYS][WARN]: busybox not found, wget wrapper not created" >> "$SERVICES_LOGS"
        return 1
    fi
    
    # Check if wget already exists in PATH
    if command -v wget >/dev/null 2>&1; then
        echo "[SYS][INFO]: wget already available in PATH" >> "$SERVICES_LOGS"
        return 0
    fi
    
    # Create wrapper if it doesn't exist or is outdated
    if [ ! -f "$WGET_WRAPPER" ] || [ ! -x "$WGET_WRAPPER" ]; then
        cat > "$WGET_WRAPPER" <<'WGET_EOF'
#!/system/bin/sh
exec busybox wget "$@"
WGET_EOF
        chmod 755 "$WGET_WRAPPER" 2>/dev/null
        
        if [ -x "$WGET_WRAPPER" ]; then
            echo "[SYS][OK]: wget wrapper created at $WGET_WRAPPER" >> "$SERVICES_LOGS"
        else
            echo "[SYS][ERROR]: failed to create wget wrapper at $WGET_WRAPPER" >> "$SERVICES_LOGS"
            return 1
        fi
    else
        echo "[SYS][INFO]: wget wrapper already exists at $WGET_WRAPPER" >> "$SERVICES_LOGS"
    fi
    
    return 0
}

# Setup wget wrapper on boot
setup_wget_wrapper
...
apply_network_ipv4_settings() {
    ...
    apply_param_set <<'EOF'
1|/proc/sys/net/ipv4/conf/all/rp_filter|rp_filter strict mode globally
1|/proc/sys/net/ipv4/conf/default/rp_filter|rp_filter strict mode by default
0|/proc/sys/net/ipv4/conf/all/accept_redirects|accept_redirects disabled globally
0|/proc/sys/net/ipv4/conf/default/accept_redirects|accept_redirects disabled by default
0|/proc/sys/net/ipv4/conf/all/accept_source_route|accept_source_route disabled globally
0|/proc/sys/net/ipv4/conf/default/accept_source_route|accept_source_route disabled by default
0|/proc/sys/net/ipv4/conf/all/send_redirects|send_redirects disabled globally
0|/proc/sys/net/ipv4/conf/default/send_redirects|send_redirects disabled by default
0|/proc/sys/net/ipv4/conf/all/secure_redirects|secure_redirects disabled globally
0|/proc/sys/net/ipv4/conf/default/secure_redirects|secure_redirects disabled by default
1|/proc/sys/net/ipv4/conf/all/src_valid_mark|src_valid_mark enabled globally
1|/proc/sys/net/ipv4/conf/default/src_valid_mark|src_valid_mark enabled by default
...
apply_network_ipv6_settings() {
    ...
    apply_param_set <<'EOF'
0|/proc/sys/net/ipv6/conf/all/disable_ipv6|IPv6 enabled globally
0|/proc/sys/net/ipv6/conf/default/disable_ipv6|IPv6 enabled by default
1|/proc/sys/net/ipv6/conf/all/accept_ra|accept_ra enabled globally
1|/proc/sys/net/ipv6/conf/default/accept_ra|accept_ra enabled by default
0|/proc/sys/net/ipv6/conf/all/accept_redirects|accept_redirects disabled globally
0|/proc/sys/net/ipv6/conf/default/accept_redirects|accept_redirects disabled by default
1|/proc/sys/net/ipv6/conf/all/use_oif_addrs_only|prefer source addresses from output interface
1|/proc/sys/net/ipv6/conf/default/use_oif_addrs_only|prefer source addresses from output interface (default)
...
start_daemon_supervisor() {
    local daemon_shell="/system/bin/sh"
    ...
    (
        while true; do
            ...
            rc=$?
            echo "[SYS][SERVICE][WARN] Daemon exited rc=$rc shell=$daemon_shell; restarting in 5s" >> "$SERVICES_LOGS"
            sleep 5
        done
    ) &
    echo "$!" > "$DAEMON_SUPERVISOR_PID_FILE"
    echo "[SYS][SERVICE] Daemon supervisor started with PID $(cat "$DAEMON_SUPERVISOR_PID_FILE" 2>/dev/null) shell=$daemon_shell" >> "$SERVICES_LOGS"
    return 0
}
